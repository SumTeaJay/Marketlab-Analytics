...
def create_data_frame_price(a: np.array, b: np.array, Q: np.array) -> pd.DataFrame:
    df = pd.DataFrame({
        "demand_intercept": a,
        "demand_slope": b,
        "quantity": Q,
...
def generate_markets() -> None:
    ...
    df_prices = create_data_frame_price(price_parameters[0], price_parameters[1], price_parameters[2])

    costs = generate_market_costs(100)
    df_costs = create_data_frame_costs(costs)
